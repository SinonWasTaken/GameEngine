	...
	public class PerlinNoise
	{
		...
		public int getNoise(int x, int range, int chunkSize = 16)
		{
			...
			while (chunkSize > 0)
			{
				...
				range = Math.Math.Max(1, range);
			}

			return Math.Math.Round(noise);
		}
		...
		public int getNoise(int x, int y, int range, int chunkSize = 16)
		{
			...
			while (chunkSize > 0)
			{
				...
				int index_x_plus = index_x + 1;// 0 ? index_x + 1 : index_x - 1;
				int index_y_plus = index_y + 1;// ? index_y + 1 : index_y - 1;
			...
			return (int) Math.Math.Round(noise);
		}
